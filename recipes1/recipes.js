constrecipes = [
	...
	function recipeTemplate(recipe) {
		const html = `<figure class="content-container">
	...
	function filterRecipes(query) {
		const filtered = recipes.filter(item => item.name.toLowerCase().includes(query) || 
	...
	function searchHandler(e) {
		e.preventDefault();
		const userInput = document.querySelector("#recipe-searchbar").value.toLowerCase();
		const sortResult = filterRecipes(userInput);
		renderRecipes(sortResult);
	}
